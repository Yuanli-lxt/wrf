...
RUN_DIR=/work/data/wps/domain_${DOMAIN_SIZE}_validation_20241001_00
...
if [[ "$(find "$GFS_DIR" -maxdepth 1 -type f -name 'gfs.t00z.pgrb2.1p00.f*' | wc -l)" -ne 25 ]]; then
  echo "Expected 25 GFS files in $GFS_DIR. Run scripts/prepare_gfs_boundary.ps1 first." >&2
  exit 1
fi
...
echo "WPS domain-size validation complete for DOMAIN_SIZE=$DOMAIN_SIZE: $RUN_DIR"
